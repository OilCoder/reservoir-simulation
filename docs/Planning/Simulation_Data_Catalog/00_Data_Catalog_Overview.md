...
This document provides a **realistic catalog** of data generated by the Eagle West Field MRST simulation scripts (s01-s20). The catalog describes the **6 primary .mat files** (~88 MB total) that contain all simulation results for machine learning model development, dashboard creation, and reservoir analysis.
...
- **File structure and content** of 6 consolidated .mat files
- **Real storage requirements** (~88 MB, not 11 GB)
...
### Primary .mat Files (6 Core Files)

| File | Size | Contributing Scripts | Content Description |
|------|------|---------------------|---------------------|
| **`grid.mat`** | ~15 MB | s03→s04→s05 | Complete PEBI geometry with 5 faults |
| **`rock.mat`** | ~12 MB | s06→s07→s08 | Rock properties with 12-layer heterogeneity |
| **`fluid.mat`** | ~18 MB | s02→s09→s10→s11 | 3-phase fluid system with PVT tables |
| **`state.mat`** | ~10 MB | s12→s13→s14 | Initial pressure/saturation distribution |
| **`wells.mat`** | ~20 MB | s15→s16 | 15-well system with completion data |
| **`schedule.mat`** | ~13 MB | s17→s18→s19 | 10-year development schedule |

**Total Storage**: ~88 MB for complete Eagle West Field model

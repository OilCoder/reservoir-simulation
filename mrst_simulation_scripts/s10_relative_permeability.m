...
% Initialize basic MRST fluid structure with canonical viscosity values
    ...
    try
        ...
        mu_o = 0.92;   % cp (canonical)
        mu_w = 0.385;  % cp (canonical)
        mu_g = 0.02;   % cp (canonical)
        fprintf('   Using fallback canonical viscosities due to config error: %s\\n', ME.message);
    end
...
% Add wettability information from SCAL data with improved error handling
    ...
    try
        if isfield(scal_config, 'wettability')
            ...
            if isfield(fluid.wettability, 'sandstone') && ...
               ...
               isfield(fluid.wettability.sandstone, 'contact_angle')
                ...
                % YAML parsing issue - use canonical values directly
                fluid.dominant_wettability = 'strongly water-wet';  % CANON for sandstone
                fluid.contact_angle = 25.0;  % CANON contact angle for sandstone
                
                % Create proper wettability structure
                fluid.wettability.sandstone.description = fluid.dominant_wettability;
                fluid.wettability.sandstone.contact_angle = fluid.contact_angle;
                fluid.wettability.sandstone.amott_harvey_oil = 0.15;
                fluid.wettability.sandstone.amott_harvey_water = 0.75;
                fluid.wettability.sandstone.wettability_index = -0.60;
                
                fprintf('   Fixed YAML parsing issue - using canonical sandstone wettability values\\n');
            end
        else
            error('Missing wettability field in SCAL configuration');
        end
    catch ME
        % Complete fallback with canonical values
        fluid.dominant_wettability = 'strongly water-wet';  % CANON for sandstone
        fluid.contact_angle = 25.0;  % CANON contact angle
        
        % Create complete wettability structure from canonical data
        fluid.wettability.sandstone.description = fluid.dominant_wettability;
        fluid.wettability.sandstone.contact_angle = fluid.contact_angle;
        fluid.wettability.sandstone.amott_harvey_oil = 0.15;
        fluid.wettability.sandstone.amott_harvey_water = 0.75;
        ...
        fprintf('   Error loading wettability config: %s\\n', ME.message);
        fprintf('   Using canonical sandstone wettability values from 04_SCAL_Properties.md\\n');
    end

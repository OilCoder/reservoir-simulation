...
% Date: August 29, 2025
    ...
    try
        ...
        step_start = tic;
        [T, state, simulation_time] = initialize_simulation(G, rock, fluid, W, initial_state, schedule_data, psia, barrel, day);
        print_step_result(2, 'Initialize Simulation', 'success', toc(step_start));
        
        % Step 3: Run simulation loop
        step_start = tic;
        results = run_simulation_loop(G, rock, fluid, W, T, state, simulation_time, schedule_data);
...
% Initialize simulation with ADI system for compressible 3-phase flow
% Units: psia (Pa), barrel (m³), day (seconds)

    % Compute transmissibility (legacy compatibility)
    ...
    % Rename x to state for ADI solver compatibility
    state = x;
    
    % Fix well controls - set producers to BHP, injectors to rate
    W = fix_well_controls(W, psia, barrel, day);
    
    % Manual wellSol initialization (initWellSol not available)
    ...
    % Validate wellSol structure for ADI solver
    if ~isfield(state, 'wellSol') || length(state.wellSol) ~= length(W)
        error('wellSol initialization failed: expected %d wells', length(W));
    end
    fprintf('   WellSol: %d wells initialized manually\n', length(state.wellSol));
    fprintf('   Solution: %d cells initialized at %.1f psi\n', length(state.pressure), realistic_pressure/psia);
...
function results = run_simulation_loop(G, rock, fluid, W, T, state, simulation_time, schedule_data)
% Execute the main simulation loop with ADI solver for realistic reservoir physics

    fprintf('   Running simulation loop with ADI solver...\n');
    
    % Initialize ADI system for 3-phase compressible flow
    try
        % Try standard ADI system initialization
        system = initADISystem({'Water', 'Oil', 'Gas'}, G, rock, fluid);
        fprintf('   ADI System: 3-phase compressible flow initialized\n');
    catch
        % Fallback to simpler 2-phase system if 3-phase fails
        system = initADISystem({'Water', 'Oil'}, G, rock, fluid);
        fprintf('   ADI System: 2-phase compressible flow initialized\n');
    end
    
    % Define timestep for simulation (monthly timesteps)
    ...
    if size(state.s, 2) >= 2
        results.saturation(:, :, 1) = state.s(:, 1:2);
    else
    ...
    for i = 1:length(W)
        ...
        fprintf("DEBUG: Accessing wellSol at line 253, step 0\n"); results.well_bhp(i, 1) = state.wellSol(i).pressure;
    end
    ...
    for step = 1:simulation_time.num_steps
        ...
        % CRITICAL FIX: Replace incompTPFA with proper ADI solver for realistic physics
        try
            [state, meta] = solvefiADI(state, dt, W, G, rock, fluid, 'system', system);
            
            % Check solver convergence
            if meta.converged
                if mod(step, 12) == 0
                    fprintf('     Timestep %d converged (iterations: %d)\n', step, meta.iterations);
                end
            else
                warning('Timestep %d failed to converge after %d iterations', step, meta.iterations);
            end
        catch ME
            % Fallback to incompressible solver if ADI fails
            warning('ADI solver failed at step %d, using incompressible fallback: %s', step, ME.message);
        ...
        % Store saturations (ADI solver updates saturations realistically)
        if size(state.s, 2) >= 2
            results.saturation(:, :, step + 1) = state.s(:, 1:2);
        else
            % Fallback if saturations not available
            results.saturation(:, :, step + 1) = results.saturation(:, :, step);
        end
        
        % Store well results
        for i = 1:length(W)
            fprintf("DEBUG: Accessing wellSol flux at step %d\n", step); results.well_rates(i, step + 1) = state.wellSol(i).flux;
            fprintf("DEBUG: Accessing wellSol pressure at step %d\n", step); results.well_bhp(i, step + 1) = state.wellSol(i).pressure;
        end
    end

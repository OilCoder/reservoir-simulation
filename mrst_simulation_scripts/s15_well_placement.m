...
% S15_WELL_PLACEMENT - 15-Well System Placement for Eagle West Field
% Requires: MRST
%
...
% Date: August 8, 2025
    ...
    try
        ...
        print_step_footer('S15', sprintf('15-Well System Placed (%d producers + %d injectors)', ...
...
% Step 1 - Load YAML configuration and grid structure
    ...
    % Also load grid and initialization configs for unit conversions
    ...
    % Make these available globally for create_well_structure function
    assignin('base', 'wells_config', config);
    assignin('base', 'grid_config', grid_config);
    assignin('base', 'init_config', init_config);
    ...
    elseif exist(base_grid_file, 'file')
        ...
        error('Grid file not found. Run s03_create_pebi_grid.m first.');
    end
...
% Step 2 - Create 10 producer wells with mixed well types
    ...
    for i = 1:length(producer_names)
        ...
        well = add_well_geometry(well, well_config, G, config);
...
% Create basic well structure with common properties
    ...
    % Initialize all possible well fields to ensure consistency (CANON-FIRST)
    % Extract wellbore radius from wells configuration
    wells_config = evalin('base', 'wells_config');
    init_config = evalin('base', 'init_config');
    grid_config = evalin('base', 'grid_config');
    ...
    wellbore_radius_m = wells_config.wells_system.completion_parameters.wellbore_radius_m;
    ...
    well.wellbore_radius = wellbore_radius_m * m_to_ft;  % CANON wellbore radius from YAML
    % Extract default skin factor from wells configuration (CANON-FIRST)
    if ~isfield(wells_config, 'wells_system') || ~isfield(wells_config.wells_system, 'completion_parameters') || ~isfield(wells_config.wells_system.completion_parameters, 'default_skin_factor')
        error(['CANON-FIRST ERROR: Missing default_skin_factor in wells_config.yaml\n' ...
               'UPDATE CANON: obsidian-vault/Planning/Wells_Configuration.md\n' ...
               'Must define exact default skin factor for Eagle West Field.']);
    end
    well.skin_factor = wells_config.wells_system.completion_parameters.default_skin_factor;
    
    % Producer-specific fields - Read from individual well configuration (CANON-FIRST)
    if isfield(well_config, 'target_oil_rate_stb_day')
        well.target_oil_rate = well_config.target_oil_rate_stb_day;
    else
        well.target_oil_rate = 0;  % Default for injectors or wells without target rate
    end
    
    % Extract production constraints from well-specific YAML configuration (CANON-FIRST)
    if isfield(well_config, 'min_bhp_psi')
        well.min_bhp_psi = well_config.min_bhp_psi;
    else
        well.min_bhp_psi = 0;  % Default for injectors
    end
    
    if isfield(well_config, 'max_water_cut')
        well.max_water_cut = well_config.max_water_cut;
    else
        well.max_water_cut = 1.0;  % Default for injectors (not applicable)
    end
    
    if isfield(well_config, 'max_gor_scf_stb')
        well.max_gor_scf_stb = well_config.max_gor_scf_stb;
    else
        well.max_gor_scf_stb = 0;  % Default for injectors (not applicable)
    end
    well.esp_type = '';
    well.esp_stages = 0;
    well.esp_hp = 0;
    
    % Injector-specific fields - Read from individual well configuration (CANON-FIRST)
    if isfield(well_config, 'target_injection_rate_bbl_day')
        well.target_injection_rate = well_config.target_injection_rate_bbl_day;
    else
        well.target_injection_rate = 0;  % Default for producers
    end
    
    % Extract injector max BHP from well-specific YAML configuration (CANON-FIRST)
    if isfield(well_config, 'max_bhp_psi')
        well.max_bhp_psi = well_config.max_bhp_psi;
    else
        well.max_bhp_psi = 0;  % Default for producers (not applicable)
    end
    
    if isfield(well_config, 'injection_fluid')
        well.injection_fluid = well_config.injection_fluid;
    else
        well.injection_fluid = '';  % Default for producers
    end
    ...
    % For PEBI grids, find the closest cell to the well location
    % Convert grid indices to physical coordinates
    % Extract field extents from grid configuration (CANON-FIRST)
    if ~isfield(grid_config, 'grid') || ~isfield(grid_config.grid, 'field_extent_x') || ~isfield(grid_config.grid, 'field_extent_y')
        error(['CANON-FIRST ERROR: Missing field_extent_x/field_extent_y in grid_config.yaml\n' ...
               'UPDATE CANON: obsidian-vault/Planning/Grid_Configuration.md\n' ...
               'Must define exact field extents for Eagle West Field.']);
    end
    field_x_extent = grid_config.grid.field_extent_x;  % CANON field extent from YAML
    field_y_extent = grid_config.grid.field_extent_y;  % CANON field extent from YAML
    
    % Convert grid indices to physical coordinates
    well_x = (I - 1) * field_x_extent / 40;  % 41 points = 40 intervals
    well_y = (J - 1) * field_y_extent / 40;
    
    % Find closest cell to well location
    distances = sqrt((G.cells.centroids(:,1) - well_x).^2 + ...
                     (G.cells.centroids(:,2) - well_y).^2);
    [~, well.cell_index] = min(distances);
...
function well = add_well_geometry(well, well_config, G, wells_config)
% Add well geometry based on well type

    % Extract completion parameters from passed configuration
    completion_params = wells_config.wells_system.completion_parameters;
    ...
    switch well_config.well_type
        ...
        case 'horizontal'
            ...
            well.kickoff_depth = well_config.total_depth_tvd_ft - completion_params.default_kickoff_depth_offset_ft;  % Canon offset
        ...
        case 'multi_lateral'
            ...
            if isfield(well_config, 'lateral_1_length_ft')
                ...
                % Extract default lateral lengths from YAML (CANON-FIRST)
                if ~isfield(completion_params, 'default_lateral_1_length_ft') || ~isfield(completion_params, 'default_lateral_2_length_ft')
                    error(['CANON-FIRST ERROR: Missing default lateral lengths in wells_config.yaml\n' ...
                           'UPDATE CANON: obsidian-vault/Planning/Well_Completions.md\n' ...
                           'Must define exact lateral lengths for multi-lateral wells in Eagle West Field.']);
                end
                well.lateral_1_length = completion_params.default_lateral_1_length_ft;
                well.lateral_2_length = completion_params.default_lateral_2_length_ft;
            end
            % Extract kickoff depth offset from YAML (CANON-FIRST)
            if ~isfield(completion_params, 'default_kickoff_depth_offset_ft')
                error(['CANON-FIRST ERROR: Missing default_kickoff_depth_offset_ft in wells_config.yaml\n' ...
                       'UPDATE CANON: obsidian-vault/Planning/Well_Completions.md\n' ...
                       'Must define exact kickoff depth offset for multi-lateral wells in Eagle West Field.']);
            end
            well.kickoff_depth = well_config.total_depth_tvd_ft - completion_params.default_kickoff_depth_offset_ft;
...
% Step 4 - Validate well locations and check for conflicts
    ...
    for i = 1:length(all_wells)
        ...
        % For PEBI grids, check if cell index is valid
        if well.cell_index < 1 || well.cell_index > G.cells.num
            validation_results.errors{end+1} = sprintf('Well %s has invalid cell index %d', well.name, well.cell_index);
        end
    end
    ...
    if ~isfield(wells_config, 'wells_system') || ~isfield(wells_config.wells_system, 'completion_parameters') || ~isfield(wells_config.wells_system.completion_parameters, 'minimum_well_spacing_ft')
    ...
    min_spacing_ft = wells_config.wells_system.completion_parameters.minimum_well_spacing_ft;  % CANON well spacing
    for i = 1:length(all_wells)-1
        for j = i+1:length(all_wells)
            ...
            if distance < min_spacing_ft
                error(['CANON-FIRST ERROR: Wells %s and %s too close: %.1f ft (minimum %.1f ft)\n' ...

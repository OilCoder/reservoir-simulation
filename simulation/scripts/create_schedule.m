...
W = addWell(W, G, rock, prod_cell, ...
    ...
    'Radius', config.wells.producer_radius * ft, ...
    'Name', config.wells.producer_name, ...
...
W = addWell(W, G, rock, inj_cell, ...
    'Type', 'rate', ...
    'Val', inj_rate_m3day, ...
    'Radius', config.wells.injector_radius * ft, ...
    'Name', config.wells.injector_name, ...
...
prod_idx = find(strcmp({schedule.control(1).W.name}, config.wells.producer_name));
inj_idx = find(strcmp({schedule.control(1).W.name}, config.wells.injector_name));

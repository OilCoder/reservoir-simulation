...
% 📊 Values from simplified configuration
fluid.sWcon = config.fluid.swcon;  % Connate water saturation
fluid.sOres = config.fluid.sores;  % Residual oil saturation
...
fluid.bO = @(p) config.fluid.oil_fvf * (1 + c_oil * (p - p_ref));   % Oil FVF
fluid.bW = @(p) config.fluid.water_fvf * (1 + c_water * (p - p_ref)); % Water FVF

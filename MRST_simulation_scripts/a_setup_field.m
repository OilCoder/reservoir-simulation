...
% Substep 1.2 – Extract grid parameters _________________________
nx = config.grid.nx;
ny = config.grid.ny;
...
% Substep 3.1 – Generate heterogeneous porosity _________________
...
poro_pattern = poro_base + poro_var * sin(2*pi*X/nx) .* cos(2*pi*Y/ny);

% Add random variation
rng(42);  % For reproducibility
poro_random = poro_var * 0.5 * (rand(ny, nx) - 0.5);
poro_field = poro_pattern + poro_random;

% Apply bounds
poro_field = max(config.porosity.min_value, min(config.porosity.max_value, poro_field));

% Convert to vector for MRST
poro_vec = reshape(poro_field', [], 1);

% Substep 3.2 – Generate permeability from porosity _____________
